          ...
          issues.length > 0 && !error.status ? issues.map(item => {
            return (
              <li key={`issues-${item.id}-${Math.random()}`} >
                <div className='hover:bg-gray-800 py-2 border-b border-slate-700 flex justify-between'>
            ...
            <li className='py-2 flex justify-center h-[100vh] items-center'>{error.message || "No results matched your search."}</li>
        }
      </ul>
    </div>
  )
